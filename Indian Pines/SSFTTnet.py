...
class SSFTTnet(nn.Module):
    def __init__(self, in_channels=1, num_classes=NUM_CLASS, num_tokens=4, dim=64, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        ...
        self.L = num_tokens
        self.cT = dim
       # -------- Small-Scale Branch (Normal Conv) --------
...
# -------- Large-Scale Branch (Dilated Conv) --------
    ...
    def forward(self, x, mask=None):
       ...
       # -------- Small Branch --------
        ...
        xs = rearrange(xs, 'b c h w y -> b (c h) w y')
        xs = self.conv2d_small(xs)

# -------- Large Branch --------
        xl = self.conv3d_large(x)
        xl = rearrange(xl, 'b c h w y -> b (c h) w y')
